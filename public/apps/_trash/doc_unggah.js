...
function ShowList(datas, page){
    ...
    $.map(datas, function(val, i){
        ...
        if(val.opk_status == 1 ){
            tr += "<td class='text-center'><div class='badge badge-success'>Menunggu Konfirmasi</div></td>";
        }else if(val.opk_status == 2 ){
            tr += "<td class='text-center'><div class='badge badge-danger'>Menunggu Pengajuan</div></td>";
        }else if(val.opk_status == 3 ){
            tr += "<td class='text-center'><div class='badge badge-danger'>Diajukan</div></td>";
        }else if(val.opk_status == 4 ){
            tr += "<td class='text-center'><div class='badge badge-danger'>Disetujui</div></td>";
        }else if(val.opk_status == 5 ){
            tr += "<td class='text-center'><div class='badge badge-danger'>Ditolak</div></td>";
        }
        tr += "<td class='text-center'><button class='btn btn-outline-primary' text='"+ val.keterangan +"'><i class='fas fa-eye'></i> Lihat</button></td>";
        tr += "<td class='text-center'>";
        tr += "<div class='btn-group btn-group-md' role='group' aria-label='Basic example'>";
        tr += "<a href='/Lbk07SrjBWMTUgfm2LIkAtxFEmpsaAfJTPi9XURsK8WQ506jx9AVUB0oLipjLsRbHKgEYu/"+val.id_folder+"' id="+val.id_folder+" class='btn btn-outline-primary' title='edit data'><i class='fas fa-upload'></i> Unggah Document</a>";
        tr += "</div>";
        tr += "</td>";
        tr += "</tr>";
